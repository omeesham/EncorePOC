...
test('test', async ({ page }) => {
  ...
  await page.goto('https://psavcustomersqa.crm.dynamics.com/main.aspx?appid=d05771f7-d483-41b2-9672-5333d8eaa013&forceUCI=1&pagetype=dashboard&type=system&_canOverride=true');
  await expect(page.getByRole('menuitem', { name: 'Refresh All' })).toBeVisible();
  await expect(page.getByRole('menuitem', { name: 'Refresh All' })).toBeVisible();
  await expect(page.getByRole('menuitem', { name: 'Refresh All' })).toBeVisible();
  await expect(page.getByLabel('Goals by Owner')).toBeVisible();
  ...
  await page.locator('iframe[title="Orders"]').contentFrame().getByText('Order Type').click();
  await page.getByRole('tabpanel', { name: 'Orders' }).click();
  await page.locator('iframe[title="Orders"]').contentFrame().locator('tr:nth-child(3) > td:nth-child(16) > div > .po-crm-grid-noWrap').click();
  await page.getByRole('tabpanel', { name: 'Orders' }).click();
  await page.locator('iframe[title="Orders"]').contentFrame().getByText('Event End Date').click();
  await page.getByRole('tabpanel', { name: 'Orders' }).click();
  await page.locator('iframe[title="Orders"]').contentFrame().getByText('Discount Amount').click();
  await page.locator('iframe[title="Orders"]').contentFrame().getByText('JBS Automation POC For Jan Order Number Order Number Order Name Net Amount').press('ArrowDown');
  await page.locator('iframe[title="Orders"]').contentFrame().getByText('JBS Automation POC For Jan Order Number Order Number Order Name Net Amount').press('ArrowDown');
  await page.locator('iframe[title="Orders"]').contentFrame().locator('html').click();
  await page.locator('#id-b8a3138e-d304-4d75-a234-866170ff8c91-126-WebResource_OrdersSubGrid-webResourceLabelControlWrapper').click();
  await page.locator('iframe[title="Orders"]').contentFrame().locator('.po-crm-List-Row.po-row-Selected > td:nth-child(16) > div > .po-crm-grid-noWrap').click();
